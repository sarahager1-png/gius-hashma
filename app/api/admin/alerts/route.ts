...
export async function GET() {
  try {
    ...
    const [jobsRes, stuckInterviewsRes, unresponsiveInstRes] = await Promise.all([
      service.from('jobs').select('id, title, created_at, institutions!inner(institution_name)').eq('status', 'פעילה'),
    ...
    const { data: appsPerJob } = await service.from('applications').select('job_id')
    const jobsWithApps = new Set((appsPerJob ?? []).map((a: { job_id: string }) => a.job_id))

    const jobsNoApplicants = (jobsRes.data ?? []).filter((j: { created_at: string; id: string }) => {
      const ageMs = now - new Date(j.created_at).getTime()
      return ageMs > 14 * 86400 * 1000 && !jobsWithApps.has(j.id)
    }).map((j: { id: string; title: string; created_at: string; institutions: unknown }) => ({
      type: 'job_no_applicants' as const,
      id: j.id,
      label: `"${j.title}" — ${(j.institutions as { institution_name: string }).institution_name}`,
    ...
    const stuckInterviews = (stuckInterviewsRes.data ?? []).map((iv: unknown) => {
      const row = iv as { id: string; created_at: string; applications: { candidates: { profiles: { full_name: string | null } | null } | null; jobs: { title: string; institutions: { institution_name: string } | null } | null } }
      ...
      return { type: 'interview_no_response' as const, id: row.id, label: `${name} לא אישרה ראיון — ${job} (${inst})`, detail: `ממתין ${hoursWaiting} שעות`, severity: 'warning' as const }
    })

    const instPendingMap: Record<string, { name: string; count: number; oldest: string }> = {}
    for (const app of unresponsiveInstRes.data ?? []) {
      const row = app as unknown as { id: string; applied_at: string; jobs: { title: string; institutions: { id: string; institution_name: string } | null } }
      const inst = row.jobs?.institutions
      if (!inst) continue
      if (!instPendingMap[inst.id]) instPendingMap[inst.id] = { name: inst.institution_name, count: 0, oldest: row.applied_at }
      instPendingMap[inst.id].count++
      if (new Date(row.applied_at) < new Date(instPendingMap[inst.id].oldest)) instPendingMap[inst.id].oldest = row.applied_at
    }
    ...
    const unresponsiveInsts = Object.entries(instPendingMap)
      ...
      .map(([id, v]) => {
        ...
        return { type: 'institution_unresponsive' as const, id, label: `${v.name} — ${v.count} הגשות ממתינות`, detail: `הוותיקה ביותר: ${daysOldest} ימים`, severity: daysOldest >= 14 ? ('critical' as const) : ('warning' as const) }
      })
