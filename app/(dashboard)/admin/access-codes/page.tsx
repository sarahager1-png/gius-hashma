...
import { LogIn, Plus, Trash2, Copy, Check } from 'lucide-react'
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-[22px] font-extrabold" style={{ color: 'var(--ink)' }}>קודי כניסה חלופיים</h1>
          <p className="text-[13px] mt-0.5" style={{ color: 'var(--ink-3)' }}>
            למועמדת או מוסד שגוגל לא עובד להם — הזיני את האימייל שלהם, צרי קוד ושלחי להם
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-3">
          <div>
            <label className="text-[12px] font-semibold block mb-1.5" style={{ color: 'var(--ink-3)' }}>
              אימייל המשתמש/ת <span style={{ color: '#DC2626' }}>*</span>
            </label>
            <input
              type="email"
              value={email}
          ...
          <div className="flex gap-3 items-end">
            <div className="flex-1">
              <label className="text-[12px] font-semibold block mb-1.5" style={{ color: 'var(--ink-3)' }}>שם לזיהוי — אופציונלי</label>
              <input
                type="text"
                value={label}
                onChange={e => setLabel(e.target.value)}
                placeholder="לדוגמה: מרים כהן — מוסד אור תורה"
                className="w-full h-10 px-3 rounded-[8px] border text-[14px] outline-none"
                style={{ borderColor: 'var(--line)' }}
                onKeyDown={e => e.key === 'Enter' && createCode()}
              />
            ...
            <button
              ...
              style={{ background: 'var(--purple)', opacity: (!email.trim() || creating) ? 0.45 : 1 }}>
              <Plus size={15} />
        ...
        <>
                ...
                {available.map(c => (
                  <div key={c.id} className="px-5 py-3.5 flex items-center gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {c.user_email && (
                        <p className="text-[12px] mt-0.5" style={{ color: 'var(--ink-2)', fontFamily: 'monospace' }} dir="ltr">{c.user_email}</p>
                      )}
                      ...
                      <p className="text-[11px] mt-0.5" style={{ color: 'var(--ink-4)' }}>
                        ...
                        {c.expires_at && ` · בתוקף עד ${fmtDate(c.expires_at)}`}
                      </p>
                    </div>
          ...
          {used.length > 0 && (
            <div className="rounded-[16px] border" style={{ background: '#fff', borderColor: 'var(--line)', boxShadow: 'var(--shadow-sm)' }}>
              <div className="px-5 py-4" style={{ borderBottom: '1px solid var(--line)' }}>
                <h2 className="text-[14px] font-bold" style={{ color: 'var(--ink-3)' }}>נוצלו ({used.length})</h2>
              </div>
              <div className="divide-y" style={{ borderColor: 'var(--line-soft)' }}>
                {used.map(c => (
                  <div key={c.id} className="px-5 py-3.5">
                    <div className="flex items-center gap-3 mb-0.5">
                      ...
                      <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: '#E4F6ED', color: '#1A7A4A' }}>נוצל</span>
                    </div>
                    {c.user_email && (
                      <p className="text-[12px]" style={{ color: 'var(--ink-2)', fontFamily: 'monospace' }} dir="ltr">{c.user_email}</p>
                    )}
                    {c.label && <p className="text-[12px] mt-0.5" style={{ color: 'var(--ink-3)' }}>{c.label}</p>}
