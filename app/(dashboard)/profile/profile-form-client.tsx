...
function AccSection({ id, title, open, onToggle, children }: { id: string; title: string; open: boolean; onToggle: () => void; children: React.ReactNode }) {
  return (
    ...
    return (
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--line)', boxShadow: 'var(--shadow-sm)' }}>
        ...
        <div className="px-6 py-6" style={{ background: 'linear-gradient(135deg, #3B1F6E 0%, #5B3AAB 100%)' }}>
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-[19px] font-black text-white leading-tight">{profileForm.full_name || '—'}</h2>
                  {candForm.specialization && <p className="text-[13px] font-semibold mt-1" style={{ color: 'rgba(255,255,255,.8)' }}>{candForm.specialization}</p>}
              </div>
            </div>
            <button onClick={() => setMode('edit')}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-[10px] text-[12px] font-bold shrink-0"
        ...
        <CardSection title="פרטים אישיים">
          <CardRow label="טלפון" value={profileForm.phone} ltr />
          <CardRow label="עיר / ישוב" value={candForm.city} />
          <CardRow label="כתובת" value={candForm.address} />
          <CardRow label="מחוז" value={candForm.district} />
          <CardRow label="שנת לידה" value={candForm.birth_year} />
          <CardRow label="מצב משפחתי" value={candForm.marital_status} />
        </CardSection>

        {(candForm.availability_from || candForm.availability_to) && (
          <CardSection title="זמינות">
            <CardRow label="מ-" value={candForm.availability_from} ltr />
            <CardRow label="עד-" value={candForm.availability_to} ltr />
          </CardSection>
        )}

        {(candForm.shlichut_location || candForm.shlichut_years || candForm.study_day) && (
          <CardSection title="שליחות">
            <CardRow label="מיקום" value={candForm.shlichut_location} />
            <CardRow label="שנים" value={candForm.shlichut_years} />
            <CardRow label="יום לימוד" value={candForm.study_day} />
          </CardSection>
        )}

        {(candForm.college || candForm.academic_level || candForm.graduation_year || candForm.seniority_years) && (
          <CardSection title="הכשרה אקדמית">
            <CardRow label="מוסד" value={candForm.college} />
            <CardRow label="רמה" value={candForm.academic_level} />
            <CardRow label="שנת סיום" value={candForm.graduation_year} />
            <CardRow label="ותק" value={candForm.seniority_years ? `${candForm.seniority_years} שנים` : ''} />
            {showExperience && <CardRow label="שנות ניסיון" value={candForm.years_experience} />}
          </CardSection>
        )}
          ...
          <CardSection title="ניסיון">
            ...
            {candForm.past_projects && (
              <div className="mt-2">
                ...
                <p className="text-[13px] whitespace-pre-wrap" style={{ color: 'var(--ink)' }}>{candForm.past_projects}</p>
              </div>
            )}
          </CardSection>
        )}
        ...
        {(candForm.bio || candForm.personal_note) && (
          <CardSection title="ביטוי אישי">
            {candForm.bio && <p className="text-[13px] leading-relaxed whitespace-pre-wrap" style={{ color: 'var(--ink)' }}>{candForm.bio}</p>}
            {candForm.personal_note && <p className="text-[12px] mt-2" style={{ color: 'var(--ink-3)' }}>{candForm.personal_note}</p>}
          </CardSection>
        )}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <button onClick={() => setMode('card')} className="flex items-center gap-1.5 text-[13px] font-semibold" style={{ color: 'var(--purple)' }}>
          ← חזרה לכרטיס
        </button>
      </div>
      ...
      <div className="rounded-[14px] px-5 py-4 flex items-start gap-3"
        ...
        <div>
          <p className="text-[13.5px] font-bold" style={{ color: '#4B2E83' }}>תוספת פרטים תעלה את רמת ההתאמה שלך למשרות</p>
          <p className="text-[12px] mt-0.5" style={{ color: '#6B5EA0' }}>רק הפרטים האישיים נדרשים. שאר הסקציות אופציונליות — פרופיל מלא מקבל עדיפות בהתאמה.</p>
        </div>
      </div>
          ...
          {workHistory.map((entry, i) => (
            <div key={i} className="rounded-[12px] p-4 space-y-3" style={{ background: '#F8F7FF', border: '1px solid #EDE9FE' }}>
              <div className="flex items-center justify-between mb-1">
                ...
                {workHistory.length > 1 && <button type="button" onClick={() => removeWork(i)} className="text-[12px] px-2 py-0.5 rounded-[6px]" style={{ color: '#B91C1C', background: '#FEE2E2' }}>הסרה</button>}
              </div>
