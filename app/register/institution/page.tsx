    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--bg-2)' }}>
      <div className="bg-white rounded-[20px] shadow-xl p-10 w-full max-w-sm text-center">
        ...
        <p className="text-[14px] leading-relaxed" style={{ color: 'var(--ink-3)' }}>
          חשבונך ממתין לאישור מנהלת המערכת.
          <br />
      ...
      <div className="w-full max-w-xl mx-auto space-y-4">
        ...
        <div className="rounded-[20px] text-center py-8 px-6"
          ...
          <p className="text-[13px] mt-1" style={{ color: 'rgba(255,255,255,.7)' }}>
            לאחר הרשמה, חשבונך יאושר על ידי מנהלת המערכת
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* פרטי כניסה */}
          {isOAuthUser ? (
            <div className="rounded-[14px] px-5 py-3 flex items-center gap-3"
              ...
              <div className="flex-1">
                <p className="text-[13px] font-bold" style={{ color: 'var(--purple)' }}>מחוברת עם Google</p>
                <p className="text-[12px]" style={{ color: 'var(--ink-3)' }} dir="ltr">{oAuthEmail}</p>
              </div>
            </div>
          ) : (
          ...
          <Section title="פרטי המוסד" icon="🏫">
            {isOAuthUser && (
              <Row label="אימייל">
                <input className={INPUT_CLS} style={{ ...INPUT_STYLE, maxWidth: '280px' }} type="email"
                  value={form.email} onChange={e => set('email', e.target.value)} dir="ltr" readOnly
                  title="האימייל מגיע מחשבון Google" />
              </Row>
            )}
            <Row label="שם המוסד">
